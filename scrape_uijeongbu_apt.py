...
def make_output_path(args):
    ...
    area  = args.sgg or (args.sido.replace('특별시','').replace('광역시','').replace('특별자치도','') if args.sido else '')
    scl   = args.scl if args.scl != '전체' else args.mcl
    flbd  = args.flbd_min.replace('전체', '전체유찰')
    parts = [p for p in [court, area, scl, f'유찰{flbd}'] if p]
...
def main():
    ...
    # 지역 후처리 필터 (사이트 지역 select가 검색에 반영 안 될 경우 대비)
    filtered = all_items
    if args.sido:
        filtered = [i for i in filtered if args.sido in (i.get('hjguSido', '') + i.get('hjguSigu', ''))]
    if args.sgg:
        filtered = [i for i in filtered if args.sgg in i.get('hjguSigu', '')]
    if len(filtered) < len(all_items):
        print(f"  지역 필터 적용: {len(all_items)}개 → {len(filtered)}개 ({args.sido or ''} {args.sgg or ''})")

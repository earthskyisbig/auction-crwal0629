...
| 7 | **권리분석(B안)** | 등기사항전부증명서 PDF(`--deungibu`) | 말소기준권리, 소멸/인수 권리 분류, 정밀 인수금 → 투자분석 인수금 대체 |
...
## 권리분석 (B안 · 등기부 PDF → 정밀 인수금)

법원 데이터엔 최선순위 1건·청구금액만 나온다. 전체 근저당·(가)압류·전세권·가처분까지 넣어 **정밀 인수금**을 내려면 **등기사항전부증명서 PDF**(인터넷등기소 발급/열람, 유료)가 필요하다. `--deungibu <pdf>` 로 제공하면:

- **`parse_deungibu.py`** (PDF→권리 목록): pdfplumber 로 텍스트 추출 → 【갑구】【을구】 분리 → 항목별 순위번호·등기목적·접수일·금액(채권최고액/전세금/보증금/청구금액)·권리자·**말소여부** 파싱. "N번○○등기말소" 참조로 이미 말소된 권리를 배제.
- **`rights_analysis.py`** (권리분석 엔진): **말소기준권리**(=소멸성 권리 중 최선순위 (근)저당·(가)압류·담보가등기·경매개시) 판정 → 각 권리를 **소멸/인수** 분류 → **정밀 인수금** 계산. 말소기준보다 앞선 **전세권·지상권·임차권·가처분·보전가등기**는 인수(전세금/보증금은 금액 합산, 가처분 등은 권리 자체 인수 경고). 명세서 대항력 임차인(전입<말소기준)도 결합.
- 결과는 리포트 **7번 「권리분석」** 섹션 + **투자분석의 인수금을 정밀값으로 대체**한다.

```bash
python3 scripts/analyze_case.py --court 서울남부지방법원 --case 2025타경9307 --deungibu ./등기부.pdf --auto-market
python3 scripts/parse_deungibu.py 등기부.pdf --dump-text     # 포맷 확인(스캔이면 텍스트 안 나옴 → OCR 필요)
python3 scripts/parse_deungibu.py --selftest                 # 합성 데이터 파싱+엔진 검증
python3 scripts/rights_analysis.py                           # 엔진 5개 시나리오 자체검증
```
> ⚠️ **검증 상태**: 엔진(5시나리오)·파싱(합성 등기부)은 자체검증 통과. **실제 인터넷등기소 PDF로는 미검증** — 실서식(다단 컬럼·표 테두리·전산이기·multi-line 등기목적)에서 pdfplumber 추출이 흔들릴 수 있어 **첫 실 PDF로 `--dump-text` 부터 확인·튜닝** 필요. 스캔 PDF는 텍스트 레이어가 없어 OCR(tesseract) 별도 구현 필요.
> ⚠️ 미포함: 유치권·법정지상권·분묘기지권 등 **등기부에 안 나오는 권리**는 현장확인. 전세권/임차권은 배당요구 시 소멸 가능(보수적 상한).
